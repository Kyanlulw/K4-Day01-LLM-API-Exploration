...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        cache = getattr(count_tokens, "_encoding_cache", {})
        enc = cache.get(model)
        if enc is None:
            # enc = tiktoken.encoding_for_model(model)
            enc_by_name = tiktoken.get_encoding("o200k_base")
            cache[model] = enc_by_name
            count_tokens._encoding_cache = cache
            print(f"[count_tokens] Loaded encoding for model {model}")
            print(f"[count_tokens] Token count for text: {len(enc_by_name.encode(text))}")
        return len(enc.encode(text))
    except Exception:
        print(f"[count_tokens] Warning: failed to load encoding for model {model}, using fallback.")
        print(f"[count_tokens] Token count for text: {max(1, len(text) // 4)}")
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    prompt_cost = prompt_tokens / 1000 * PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])["input"]
    completion_cost = completion_tokens / 1000 * PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])["output"]
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        print()  # Newline after assistant's reply
        history.extend([{"role": "user", "content": user_msg},
                        {"role": "assistant", "content": "".join(reply_parts)}])
        history = history[-8:]  # Keep last 4 turns (8 messages)
...
def run_assistant(
    ...
    while True:
        ...
        history = history[-8:]
        turns += 1
        tokens_used += count_tokens(user_msg, OPENAI_MODEL) + count_tokens(
            reply, OPENAI_MODEL
        )
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    """
    return [{**compare_models(prompt), "prompt": prompt} for prompt in prompts]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        row = [
            ...
            shorten(result.get("gpt4o_answer", "")),
            shorten(result.get("mini_answer", "")),
            f'{float(result.get("gpt4o_time", 0.0)):.2f}s',
            f'{float(result.get("mini_time", 0.0)):.2f}s',
        ]
        rows.append(row)
